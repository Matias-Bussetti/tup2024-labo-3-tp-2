...
public class ClienteServiceTest {
    ...
    public void testComentario1() throws TipoCuentaAlreadyExistsException {
        ...
        Cuenta cuentaCA = new Cuenta()
                .setMoneda(TipoMoneda.PESOS)
                .setBalance(500000)
                .setTipoCuenta(TipoCuenta.CA$);
        Cuenta cuentaCC = new Cuenta()
                .setMoneda(TipoMoneda.PESOS)
                .setBalance(500000)
                .setTipoCuenta(TipoCuenta.CC$);

        when(clienteDao.find(dni, true)).thenReturn(cliente);

        clienteService.agregarCuenta(cuentaCA, dni);
        clienteService.agregarCuenta(cuentaCC, dni);
        // clienteService.agregarCuenta(cuentaCA, dni);
    ...
    public void testComentario2() throws TipoCuentaAlreadyExistsException {
        ...
        Cuenta cuentaCAU = new Cuenta()
                .setMoneda(TipoMoneda.PESOS)
                .setBalance(500000)
                .setTipoCuenta(TipoCuenta.CAU$S);

        when(clienteDao.find(dni, true)).thenReturn(cliente);

        clienteService.agregarCuenta(cuentaCA, dni);
        clienteService.agregarCuenta(cuentaCAU, dni);
        // clienteService.agregarCuenta(cuentaCA, dni);
